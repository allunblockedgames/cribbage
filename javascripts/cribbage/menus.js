...
function MenuCardAppear(elementID, skipAnimation) {
  ...
  with (el.style) {
    if (!skipAnimation) {
      WebkitTransition =
        ...
        OTransition =
        msTransition =
          "0.8s cubic-bezier(0.175, 0.885, 0.320, 1.275)";
    }
    top = "25%";
    opacity = 1;
    pointerEvents = "auto";
...
function ShowStartAGameMenu(skipAnimation) {
  //   var menuName = visibleMenuCards[visibleMenuCards.length - 1];
  //   MenuCardPressDown(menuName);
  MenuCardAppear("menu_start_a_game", skipAnimation);
}
...
function InitializeSettingsView() {
  document.getElementById("setting_manually_count_scores_checkbox").checked =
    GetSetting("setting_manual_count_scores");
  document.getElementById("setting_muggins_checkbox").checked =
    GetSetting("setting_muggins");
  document.getElementById("setting_hints_checkbox").checked =
    GetSetting("setting_hints");
  document.getElementById("setting_warn_suboptimal_checkbox").checked =
    GetSetting("setting_warn_suboptimal");
  document.getElementById("setting_fast_count_checkbox").checked =
    GetSetting("setting_fast_count");

  var board_color = GetSetting("setting_board_color");
  var allElems = document.getElementsByName(
    "settings_boardbackground_selector"
  );
...
function ShowStatisticsMenu(e) {
  if (e) {
    e.preventDefault();
  }

  InitializeStatisticsView();
  if (window.cribbageApi && window.cribbageApi.showStatisticsMenu) {
    window.cribbageApi.showStatisticsMenu();
  }
}
...
function InitializeStatisticsView() {
  ...
  for (var i = 0; i < difficulties.length; i++) {
    ...
    var gamesPlayedElement = document.getElementById(
      "menu_stat_games_played_" + curDifficulty
    );
    var winsElement = document.getElementById(
      "menu_stat_wins_" + curDifficulty
    );
    var lossesElement = document.getElementById(
      "menu_stat_losses_" + curDifficulty
    );
    var skunksElement = document.getElementById(
      "menu_stat_skunks_" + curDifficulty
    );
    var winPercentElement = document.getElementById(
      "menu_stat_win_percent_" + curDifficulty
    );
    if (gamesPlayed > 0) {
      ...
      winPercentElement.innerText =
        parseFloat((100 * wins) / gamesPlayed).toFixed(0) + "%";
    } else {
  ...
  if (totalGamesPlayed > 0) {
    ...
    winPercentElement.innerText =
      parseFloat((100 * totalWins) / totalGamesPlayed).toFixed(0) + "%";
  } else {
  ...
  for (var i = 0; i < avgCategories.length; i++) {
    ...
    for (var j = 0; j < difficulties.length; j++) {
      ...
      var element = document.getElementById(
        "menu_" + curCategory + "_" + difficulty
      );
      if (peggingRoundsCount == 0) {
        element.innerText = "";
      } else {
        element.innerText = parseFloat(
          peggingRoundsTotal / peggingRoundsCount
        ).toFixed(1);
      }
    }
    var element = document.getElementById(
      "menu_" + curCategory + "_" + "Total"
    );
    if (totalCount == 0) {
      element.innerText = "";
    } else {
...
function InitializeSubOptimalHistoryView() {
  ...
  for (
    var curError = 0;
    curError <= maxError;
    curError = curError + linesErrorSpacing
  ) {
    var line = document.createElementNS("http://www.w3.org/2000/svg", "line");
    line.classList.add("msoHLine");
    var curY =
      graphHeight - graphPadding[3] - (curError / maxError) * plotRegionHeight;
  ...
  if (history.length == 1) {
    ...
    for (var i = 0; i < history.length; i++) {
      data =
        ...
        (graphHeight -
          graphPadding[3] -
          (history[i] / maxError) * plotRegionHeight) +
        " ";
      curLeft = curLeft + xSpacing;
    }
    var historyLine = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "polyline"
    );
    ...
    lastScore.setAttribute(
      "x",
      graphPadding[0] + xSpacing * (history.length - 1) + 2
    );
    lastScore.setAttribute(
      "y",
      graphHeight -
        graphPadding[3] -
        (history[history.length - 1] / maxError) * plotRegionHeight
    );
...
function InitializeDiscardAnalyzer() {
  ...
  cribText.innerText = discardAnalyzerIsPlayersCrib
    ? "Your Crib"
    : "Opponent's Crib";
...
function PrepareDiscardAnalyzerButtons() {
  ...
  if (daSelectedCells.length == 6) {
    var optimalStats = game.FindOptimalCribDiscards(
      cards,
      discardAnalyzerIsPlayersCrib
    );
    ...
    optimalScoreText.innerHTML =
      "avg score: " + parseFloat(optimalStats[1]).toFixed(1) + " pts";
    ...
    if (cardsLeftToClick == 1) {
      ...
      prompt.innerHTML =
        "Click on " + cardsLeftToClick + " cards to find the optimal discards:";
    }
  }
}
...
function InitializeAllPlays(cards, playedCards, isCribScorePositive) {
  ...
  for (var i = 0; i < cards.length; i++) {
    for (var j = i + 1; j < cards.length; j++) {
      ...
      var scoreStats = game.GetScoreStatsForPossibleDiscards(
        trialCards,
        cribCards,
        isCribScorePositive
      );
  ...
  for (var i = 0; i < allPlays.length; i++) {
    ...
    playView.children[12].children[3].innerText = parseFloat(
      allPlays[i].avgScore
    ).toFixed(2);

    if (
      playedCards !== null &&
      ((playView.play.cribCards[0] === playedCards[0] &&
        playView.play.cribCards[1] === playedCards[1]) ||
        (playView.play.cribCards[0] === playedCards[0] &&
          playView.play.cribCards[1] === playedCards[1]))
    ) {
...
function mallcellClick(playView) {
  InitializeHandAnalysis(
    playView.play.handCards,
    playView.play.cribCards,
    playView.play.isPlayersCrib
  );
  var menuName = visibleMenuCards[visibleMenuCards.length - 1];
  MenuCardPressDown(menuName);
...
function InitializeHandAnalysis(handCards, cribCards, isCribScorePositive) {
  ...
  for (var i = 0; i < handCards.length; i++) {
    document.getElementById("mhacard" + i).style.background =
      handCards[i].image;
  }
  cribCards.sort(function (a, b) {
    return a.rank - b.rank;
  });
  ...
  document.getElementById("mhaAvg").innerHTML = parseFloat(
    scoreStats[1]
  ).toFixed(2);
  document.getElementById("mhaMax").innerHTML = scoreStats[2];

  document.getElementById("mhacribtitle").innerHTML = isCribScorePositive
    ? "Your crib"
    : "Opp. crib";
  ...
  for (var i = 0; i < deck.length; i++) {
    var topCard = deck[i];
    if (
      handCards.indexOf(topCard) === -1 &&
      cribCards.indexOf(topCard) === -1
    ) {
  ...
  var histBarWidth =
    (histWidth - histPadding[0] - histPadding[2]) / (maxScore - minScore + 1);
  ...
  for (var i = minScore; i <= maxScore; i++) {
    if (histogramCounts[i] === undefined) {
      ...
      var curHeight =
        curHeightPercent * (histHeight - histPadding[1] - histPadding[3]);
      if (curHeight < 1) {
        barDiv.style.height = "1px";
        barDiv.style.top =
          histPadding[1] +
          (histHeight - histPadding[1] - histPadding[3]) -
          1 +
          "px";
      } else {
